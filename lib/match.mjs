...
// Совпадение по ЦЕЛЫМ словам — чтобы короткие коды таймзон ("uk", "cet",
// "gmt") не срабатывали внутри других слов (напр. "uk" в "Milwaukee").
...
export function evaluateJob(job) {
  ...
  // Таймзона: совпала, если целым словом упомянут твой регион (в локации
  // или описании). "Гибкая" — если в ЛОКАЦИИ есть worldwide/anywhere/global.
  const timezoneMatch = matchesAnyWord(
    `${job.location} ${haystackFull}`,
    criteria.timezones
  );
  const timezoneFlexible = includesAny(
    job.location,
    criteria.timezoneFlexibleKeywords
  );

  // Жёсткий фильтр 5: только твой часовой пояс (если включено в criteria).
  // Отсекает US-вакансии и «безрегиональные» — работать в ночь по US не нужно.
  if (criteria.timezoneRequired && !timezoneMatch && !timezoneFlexible) {
    return null;
  }
