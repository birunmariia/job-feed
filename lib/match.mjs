...
function isRemote(job, haystackFull) {
  if (REMOTE_SOURCES.includes(job.source)) return true;
  return includesAny(`${job.location} ${haystackFull}`, criteria.remoteKeywords);
}
...
export function evaluateJob(job) {
  ...
  if (criteria.remoteOnly && !isRemote(job, haystackFull)) return null;
